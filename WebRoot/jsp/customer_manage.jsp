...
<body>
	...
	<div id="win" class="easyui-window" title="客户数据编辑"
		...
		<form id="editForm" method="post">
			<%--提供id隐藏域 --%>
			<input type="hidden" name="id"> 客户姓名：<input id="name"
				...
				name="gender" value="女" />女 <span id="genderMessage"
	...
	<script type="text/javascript">
					...
					function() {
						...
						$.messager.confirm("提示", "确认删除数据吗?", function(value) {
							if (value) {
								...
								//传递到后台
								$.post("../customer/delete.action", idStr,
										function(data) {
											if (data.success) {
												//刷新datagrid
												$("#list").datagrid("reload");
		...
		/* function isChecked() {
  		    ...
  		    $("input:radio[name='gender']:checked").each(
  		        function () {
